...
class MonsterInsights_Email_Summaries {
	...
	public function hooks() {
		...
		if ( ! empty( $this->email_options['email_summaries'] ) && 'on' === $this->email_options['email_summaries'] ) {
			...
			add_action( 'monsterinsights_after_update_settings', array( $this, 'reset_email_summaries_options' ), 10, 2 );
		}
	...
	public function get_email_subject() {
		return apply_filters( 'monsterinsights_emails_summaries_cron_subject', esc_html__( 'MonsterInsights Summary', 'ga-premium' ) );
	}
	...
	public function send_test_email() {
		...
		$email 			 	= array();
		$email['subject']  	= '[Test Email] ' . $this->get_email_subject();
		$email['address'] 	= $this->get_email_addresses();
		$email['address']  	= array_map( 'sanitize_email', $email['address'] );
